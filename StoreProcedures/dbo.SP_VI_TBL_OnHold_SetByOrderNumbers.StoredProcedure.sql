	...
	BEGIN TRY
		...
		BEGIN
			IF EXISTS (
					...
					FROM dbo.Fulfilment F
						...
						INNER JOIN @OrderNumbers N ON N.OrderNumber = F.order_number
					WHERE ISNULL(F.Deleted, 0) = 0
						AND ISNULL(F.OnHold, 0) = 0
						AND LOWER(ISNULL(N.OrderSource, '')) = 'manual'
						AND (w.StepStatus IS NULL OR w.StepStatus NOT IN ('Started', 'Completed'))
					)
			BEGIN
				SET @Message = 'There is some orders added to wave. Please refresh and try again.';				
				RETURN;
			END
		END		
        ...
        BEGIN
			...
			WHILE @@FETCH_STATUS = 0
				...
				SET @FulfilmentOrder = CAST(@FulfilmentOrderID AS VARCHAR(21));
				--RIGHT('000000000000000000000' + CAST(@FulfilmentOrderID AS varchar), 21);
